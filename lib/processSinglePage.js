...
// ------------------------------------------------------------------
// Tools array: Original function definitions for OpenAI
// ------------------------------------------------------------------
const tools = [
  {
    type: "function",
...
// ------------------------------------------------------------------
function truncateContent(content, mediaLinks, maxTokens = 90000) {
...
// ------------------------------------------------------------------
async function generateStructuredFAQs(title, content, rawTimestamp, images) {
  let lastError = null;
  ...
  for (let attempt = 0; attempt < retryAttempts; attempt++) {
    try {
      ...
      console.log(`[generateStructuredFAQs] Attempt ${attempt + 1}/${retryAttempts} for "${title}"`);
...
`
  ...
  console.error(`[generateStructuredFAQs] ❌ All attempts failed for "${title}".`);
  return false;
}
...
// ------------------------------------------------------------------
async function generateAdditionalFAQs(title, content, existingFAQs, images) {
  let lastError = null;
  const retryAttempts = 3;
...
${unusedImages.map((url, index) => `[Image ${index + 1}]: ${url}`).join('\n')}
      ...
      console.log(`[generateAdditionalFAQs] 🟡 Calling OpenAI for additional FAQs on "${title}"...`);
      const startTime = Date.now();
      const [response] = await Promise.all([
        openai.chat.completions.create({
          model: "gpt-4o-mini",
...
`
      ...
      const args = JSON.parse(toolCall.function.arguments);
      console.log(`[generateAdditionalFAQs] Received from OpenAI for "${title}":`, JSON.stringify(args, null, 2));
...
// ------------------------------------------------------------------
function formatCrossLinks(faqs) {
...
// ------------------------------------------------------------------
function formatWikipediaSlug(title) {
  return title.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase();
}

async function saveMetadata(title, humanReadableName) {
  ...
  try {
    ...
    if (existingEntry) {
      console.log(`[saveMetadata] Found existing entry for "${slug}", ID ${existingEntry.id}`);
...
// ------------------------------------------------------------------
async function saveStructuredFAQ(title, url, humanReadableName, lastUpdated, faqs) {
  if (!url) {
  ...
  if (!faqFileId) {
    console.error(`[saveStructuredFAQ] ❌ Failed to get or create FAQ file for "${title}".`);
    return;
  }
  ...
  for (const link of allCrossLinks) {
    ...
    try {
      ...
      if (!existing) {
        ...
        console.log(`[saveStructuredFAQ] ✅ Queued cross-link: ${crossLinkTitle}`);
      }
    } catch (error) {
      console.error(`[saveStructuredFAQ] Error queueing cross-link ${crossLinkTitle}:`, error);
    }
  }
...
Related Pages: ${relatedPages}
      ...
      const vector = {
        ...
        metadata: {
          ...
          media_link: mediaUrl || "",
          image_urls: faq.media_links ? faq.media_links.map(u => u.trim()) : []
        }
      };
  ...
  if (vectors.length > 0) {
    console.log(`[saveStructuredFAQ] 🟡 Upserting ${vectors.length} embeddings to Pinecone...`);
    try {
      await index.upsert(vectors);
      console.log(`[saveStructuredFAQ] ✅ Upserted ${vectors.length} FAQs to Pinecone.`);
  ...
  console.log(`[saveStructuredFAQ] ✅ Completed saving ${faqs.length} first-pass FAQs for "${title}".`);
}

// ------------------------------------------------------------------
// Save second-pass (additional) FAQs
// ------------------------------------------------------------------
async function saveAdditionalFAQs(title, additionalFaqs, url, humanReadableName, lastUpdated) {
  ...
  for (const link of allCrossLinks) {
    ...
    const crossLinkUrl = `https://en.wikipedia.org/wiki/${link}`;
    try {
      // If you want cross-links queued:
      const { data: existing } = await supabase
        .from("processing_queue")
        .select("id")
        .eq("slug", crossLinkSlug)
      ...
      if (!existing) {
        ...
        console.log(`[saveAdditionalFAQs] ✅ Queued cross-link: ${crossLinkTitle}`);
      }
    } catch (error) {
      console.error(`[saveAdditionalFAQs] Error queueing cross-link ${crossLinkTitle}:`, error);
    }
  }
...
Related Pages: ${relatedPages}
      ...
      console.log(`[saveAdditionalFAQs] Generating embedding for: "${faq.question}"`);
      const embedding = await generateEmbedding(embeddingText);
  ...
  if (vectors.length > 0) {
    ...
    try {
      ...
      if (updateError) {
        console.error(`[saveAdditionalFAQs] ❌ Error marking pinecone_upsert_success:`, updateError.message);
      } else {
        console.log(`[saveAdditionalFAQs] ✅ Marked pinecone_upsert_success=true for ${justUpsertedIds.length} rows.`);
      }
    } catch (upsertError) {
...
// ------------------------------------------------------------------
async function fetchWikipediaMetadata(title) {
...
export async function processOnePageFromDB(id) {
  ...
  if (record.status !== "pending") {
    console.log(`[processOnePageFromDB] Row ${id} not in "pending" status. Current status: ${record.status}`);
    return false;
  }
  ...
  //    - record.title might be "Auckland_Zoo", or "Some_Page"
  //    - or if you have a "slug", adjust as needed
  const { title } = record;
  ...
  if (!pageData) {
    console.error(`[processOnePageFromDB] ❌ No Wikipedia content for "${title}". Marking failed.`);
    await supabase
      .from("processing_queue")
      .update({
        status: "failed",
  ...
  // 4) First Pass
  console.log(`[processOnePageFromDB] Generating structured FAQs (first pass) for "${title}"...`);
  const structured = await generateStructuredFAQs(title, content, lastUpdated, images);
  if (!structured) {
    console.error(`[processOnePageFromDB] ❌ First pass failed for "${title}". Marking failed.`);
    await supabase
      .from("processing_queue")
      .update({
        status: "failed",
  ...
  // 5) Second Pass
  console.log(`[processOnePageFromDB] Generating additional FAQs (second pass) for "${title}"...`);
  const additionalFaqs = await generateAdditionalFAQs(title, content, faqs, images);
  if (additionalFaqs && additionalFaqs.length > 0) {
    console.log(`[processOnePageFromDB] Saving ${additionalFaqs.length} additional FAQ(s) for "${title}"...`);
    await saveAdditionalFAQs(title, additionalFaqs, pageUrl, finalName, finalTimestamp);
  } else {
