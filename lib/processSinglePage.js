...
`
      ...
      //
      // 1) OpenAI usage object (if returned)
      console.log("[generateStructuredFAQs] openaiResponse usage:", response.usage || "No usage data");
      // 2) Possibly the request ID in headers, if the library populates it
...
export async function saveStructuredFAQ(
  ...
  // Now fetch the newly-created row from faq_files:
...
Related Pages: ${relatedPages}
  ...
  // Upsert everything in chunks
  if (vectors.length > 0) {
    console.log(
      `[saveStructuredFAQ] 🟡 Preparing to upsert ${vectors.length} embeddings to Pinecone in chunks...`
    );

    // Decide how many vectors to send per upsert call:
    const CHUNK_SIZE = 50; // or 20, or 100, etc.
    const justUpsertedIds = [];

    try {
      // Loop in increments of CHUNK_SIZE
      for (let i = 0; i < vectors.length; i += CHUNK_SIZE) {
        const chunk = vectors.slice(i, i + CHUNK_SIZE);
        console.log(
          `[saveStructuredFAQ] 🟡 Upserting chunk [${i}..${
            i + chunk.length - 1
          }] (size ${chunk.length}) to Pinecone...`
        );

        await index.upsert(chunk); // Upsert just this chunk
        console.log(`[saveStructuredFAQ] ✅ Chunk of ${chunk.length} upserted.`);

        // Collect IDs for marking pinecone_upsert_success
        for (const v of chunk) {
          justUpsertedIds.push(parseInt(v.id, 10));
        }
      }

      // After all chunks are done, mark success in Supabase
      ...
      console.error(
        `[saveStructuredFAQ] ❌ Pinecone upsert failed:`,
        upsertError.message
      );
    }
  } else {
