...
export const functions = [
  {
    ...
    description:
      "You are the professor of the subject of this wikipedia page, provide a list of at least 5 questions and answers that are the most important elements of this subject. Make sure that you answer with details, like names, dates, places, references, and so forth. If there are any image URLs in the HTML that would enrich the question or answer, make sure to include those URLs. Do NOT change the case of Wikipedia page titles or cross-links. Make sure we have broad coverage of the subject, there should be a minimum of one question for every section within the Wikipedia page.",
    parameters: {
      type: "object",
      properties: {
        title: {
          ...
          items: {
            ...
            properties: {
              ...
              answer: {
                type: "string",
                description:
                  "The answer to the question. These should be rich with facts and data, but also written in an engaging manner that would appeal to a wide audience. They should have a minimum of 3 sentences of content and ideally 10 sentences of content, but no filler language, just facts unique to the question. There should be no links to other pages in this answer text. Don't include links to other pages in the answer."
              },
              cross_links: {
                type: "array",
                items: {
                  type: "string",
                  description:
                    "If the wikipedia html content where we got the question and answer has links to other wikipedia pages, include the exact name of the page with the same case and underscores. These are references to relevant pages on Wikipedia to the question and answer, but are not the page we are getting content from. Also don't include anchor links or redirects, only unique pages that exist. Do not use links that say: (Redirected from <link>) because they don't have Wikipedia pages. Don't provide links that mention redirects. Don't include the portion before the slash /. For instance, it should be Pro_Football_Hall_of_Fame, not /wiki/Pro_Football_Hall_of_Fame. Do not include anchor links (e.g., Auckland_Zoo#Major_exhibits)"
                }
              },
              media_links: {
                type: "string",
                description:
                  "If the HTML we provide has a reference to an image url in the section where we are getting the question/answer pair, include it here. If there are more than one images, try to associate the image that best fits the question or answer content. Extract images from this HTML page that are relevant to the content, especially those that appear in infoboxes, have captions, or are referenced in the metadata (og:image).The link should start with https://. Do not provide a { or other wrapping around the media_link URL. Many of wikipedias image urls will have the .jpg repeated twice in the url and that is ok. DO NOT MAKE UP A LINK, IT MUST BE EXACTLY AS FOUND IN THE HTML. If there is no image url, just leave this blank."
              }
            },
  ...
  },
  {
    ...
    description:
    "This is the second run of the same function. We have included the questions and answers we generated from the first pass so you don't repeat those. You are a subject-matter expert on this Wikipedia page. Generate a list of at least 5 insightful and **varied** questions that go beyond simple summaries or lists. Ensure the questions cover different types of inquiries, including:\n" +
    "- **Comparative**: 'How did X’s theories differ from Y’s?'\n" +
    "- **Predictive**: 'If X had not been exiled, how might their influence on literary theory have changed?'\n" +
    "- **Hypothetical**: 'What challenges would X face if their ideas were introduced in the modern academic landscape?'\n" +
    "- **Scenario-based**: 'If X and Y had debated each other, what might their key arguments have been?'\n" +
    "Ensure at least one question per major section of the Wikipedia page.",
    parameters: {
      type: "object",
      properties: {
        title: {
          ...
          items: {
            ...
            properties: {
              ...
              media_links: {
                type: "string",
                description:
                  "If the HTML we provide has a reference to an image url include it here. If there are more than one images, try to associate the image that best fits the question or answer content. Extract images from this HTML page that are relevant to the content, especially those that appear in infoboxes, have captions, or are referenced in the metadata (og:image). The link should start with https://. Do not provide a { or other wrapping around the media_link URL. Many of wikipedias image urls will have the .jpg repeated twice in the url and that is ok. DO NOT MAKE UP A LINK, IT MUST BE EXACTLY AS FOUND IN THE HTML. If there is no image url, just leave this blank."
              }
            },
            required: ["subheader", "question", "answer"]
          },
...
export async function generateEmbedding(text) {
  try {
    console.log("[generateEmbedding] Generating embedding");
          ...
          messages: [
            {
              role: "system",
              content:
                "You are the professor of the subject of this wikipedia page, provide a list of at least 5 questions and answers that are the most important elements of this subject. Make sure that you answer with details, like names, dates, places, references, and so forth. Do NOT change the case of Wikipedia page titles or cross-links"
            },
            {
              role: "user",
              content: `You are the professor of the subject of this wikipedia page, provide a list of at least 5 questions and answers that are the most important elements of this subject. Make sure that you answer with details, like names, dates, places, references, and so forth. Do NOT change the case of Wikipedia page titles or cross-links. Content includes:
...
export async function saveStructuredFAQ(
  ...
  // Prepare to insert each FAQ into the database and generate embeddings for them
      for (const faq of faqs) {
        try {
          ...
          // Get the media URL from the OpenAI response.
          ...
          // Call your isBrokenMedia function to check if the URL returns a valid page.
          // (This uses the same technique as your standalone script.)
          if (mediaUrl) {
            const broken = await isBrokenMedia(mediaUrl);
            if (broken) {
              console.log(`[saveStructuredFAQ] Media URL is broken: ${mediaUrl}. It will not be saved.`);
              mediaUrl = null;
            }
          }

          // Build your FAQ data object, including the (possibly null) media_link.
          ...
          const faqData = {
            ...
            media_link: mediaUrl  // Only valid media URLs make it here.
          };

          console.log("[saveStructuredFAQ] FAQ data (before insert)");
  ...
  Related Pages: ${relatedPages}
      ...
      console.log("[saveStructuredFAQ] Prepared vector for Pinecone upsert");
      vectors.push(vector);
    } catch (err) {
...
export async function processOnePageFromDB(id) {
  ...
  if (error) {
    ...
    return { success: false, reason: `Database fetch error: ${error.message}` };
  }
  if (!record) {
    console.error(`[processOnePageFromDB] ❌ No record found with ID=${id}.`);
    return { success: false, reason: `No record found with ID=${id} in 'processing_queue'` };
  }
  ...
  if (record.status !== "pending") {
    ...
    return { success: false, reason: `Status is "${record.status}", not "pending".` };
  }
  ...
  // Suppose wikiUrl = "https://en.wikipedia.org/wiki/The_Chicks" or similar.
  ...
  try {
    ...
    await supabase
      ...
      .eq("id", record.id);
    return { success: false, reason: `Malformed wikiUrl: ${wikiUrl}` };
  }
  ...
  if (!pageData) {
    ...
    await supabase
      ...
      .eq("id", record.id);
    return { success: false, reason: `No Wikipedia content found for "${wikiPageParam}"` };
  }
  ...
  if (!structured) {
    ...
    await supabase
      ...
      .eq("id", record.id);
    return { success: false, reason: "First pass generation returned no data" };
  }
