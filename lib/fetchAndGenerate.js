...
const MEDIA_PAGE_LIMIT = parseInt(process.env.MEDIA_PAGE_LIMIT || "10", 10);
...
const generateStructuredFAQs = async (title, content, rawTimestamp, images) => {
  ...
  for (let attempt = 0; attempt < retryAttempts; attempt++) {
    try {
      ...
      // Fix: change "\\n" to '\n' to avoid the "Expected unicode escape" error
      const contentWithImages = `
        ${truncatedContent}
        Relevant Images:
        ${truncatedMediaLinks.map((url, index) => `[Image ${index + 1}]: ${url}`).join('\n')}
...
const generateAdditionalFAQs = async (title, content, existingFAQs, images) => {
  ...
  for (let attempt = 0; attempt < retryAttempts; attempt++) {
    try {
      ...
      // Fix: change .join('\\n') to .join('\n') to avoid parse issues
      const existingQuestions = existingFAQs.map(faq => 
        `- ${faq.question}\n  Subheader: ${faq.subheader}\n  Used images: ${(faq.media_links || []).join(", ")}`
      ).join('\n');

      // Also fix: change .join("\\n") to .join('\n')
      const contentWithImages = `
        ${truncatedContent}
...
async function startProcess() {
  ...
  if (!openai || !supabase) {
    // If something failed, log it and then just RETURN or THROW
    console.error("[startProcess] ❌ One or more clients failed to initialize.");
    return; 
    // or: throw new Error("[startProcess] Failed to initialize clients.");
  }
  ...
  try {
    ...
    // Instead of process.exit(0), just return
    return;
  } catch (error) {
