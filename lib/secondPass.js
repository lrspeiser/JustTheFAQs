...
export async function generateAdditionalFAQs(title, content, existingFAQs, images) {
  ...
  for (let attempt = 0; attempt < retryAttempts; attempt++) {
    try {
      ...
      // 2) Filter out images already used
...
`
      ...
      console.log("[generateAdditionalFAQs] openaiResponse headers:", response.headers || "No headers present");
      console.log("[generateAdditionalFAQs] openaiRequestId:", response.headers?.["openai-request-id"] || "No request ID");
      console.log("[generateAdditionalFAQs] openaiResponse choices:", response.choices || "No choices returned");
